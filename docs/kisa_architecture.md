...
> **▶ RESUME HERE (next action):** Part 1 **Ch 1 (5) + Ch 2 (16) = 21 narrations are done, unified, committed** (`cdd399b`) and rendering correctly. **Next is Ch 3** ("Recognition of the Scholar…", PDF p.26+) — **but wait for the user's explicit go-ahead**; the user intends to `git push` Ch 1–2 to production first. To continue: re-read this subsection (the conventions + pipeline below), then for Ch 3 render PDF pages ~26+ with PyMuPDF, read the Arabic/English visually, translate, and append to `basair_complete.json` using the same `manual_chain`/`manual_body` format. Verify in the app (dev server, `/hadith/basair-al-darajat/...`).
