...
> **▶ RESUME HERE (next action):** Part 1 **Ch 1–5 = 40 narrations done** (Ch 5 was the first team-sourced chapter — drafted on claude.ai Sonnet via the translator kit, gate-corrected + ingested; see Progress) (repo is now `github.com/hadimh99/Kisa`). **All leads follow the scholar's closest-to-the-Imam rule** (see conventions below); **1-1-5 is a non-Imam report** (`nonImamReport` flag + UI footnote). **Next is Ch 6** (PDF p.33+) — **wait for the user's explicit go-ahead**. **Team workflow now live:** per-chapter PDFs in `~/Documents/Basair-Chapters/`, kit in `docs/basair-translator-kit.md`, dedup via `ASSIGNMENT-TRACKER.csv`; ingest team output per "Ingesting team translations". To continue: re-read this subsection (the conventions + pipeline below), then render PDF pages with PyMuPDF (`venv/bin/python`, `fitz.Matrix(4.5,4.5)` zoomed band crops read cleanly), read the Arabic/English visually, translate, and append using the same `manual_chain`/`manual_body` format (apply ALL the translation conventions below — lead rule, honorifics, kunya table, plain English, mithl cross-ref, non-Imam footnote). Verify in the app (`/hadith/basair-al-darajat/vol-1/part-1/<slug>`; slug = `slugifyHadithParam(chapter)` from `HadithLibrary.jsx`). **Dev gotcha:** the Claude preview proxy can't reach vite (vite ignores the assigned PORT and binds 5173/5174) — drive **`http://localhost:5173`** directly for visual checks.
...
- **⚠️ ARABIC NEEDS SCHOLAR PROOFING (user will do this once the book is done):** all `arabicText` was transcribed by visually reading the rendered page images (the PDF text layer is corrupted), so it needs an Arabic-proofing pass before it can be considered authoritative. Known watch-items so far: **Ch2 H3 & H7** sit across page breaks (partly reconstructed); **H7 angel count** ("a thousand" per hubeali vs possibly "seventy thousand"); isnad name **al-Mufaddal B. Salih vs "Salim"**; **Ch3 isnad names** — `al-Husayn B. Saeed` (hubeali shows "al-Saeed"; normalized to house spelling), `Muhammad B. al-Husayn B. Sagheer`, `Umar B. Qays al-Masir`, and H2's marfu' note ("raising it" = yarfaʿuhu); general diacritics. **Ch4 watch-items:** **H3** "وَ عَنْهُ بِهَذَا الْإِسْنَادِ قَالَ" is ambiguous — rendered as the Imam(s)' own words by the same chain as H2 (al-Sadiq from his father al-Baqir), since the bare "قال" lacks "قال رسول الله"; confirm whether the speaker is al-Baqir (current reading) or a continuation of the Prophet's words. **H10** isnad reads `al-Husayn B. Mahboub` per the page image + hubeali, but the well-known transmitter that Ahmad b. Muhammad relays from is **al-Hasan B. Mahboub al-Sarrad** — likely a scribal `al-Husayn`/`al-Hasan` slip; transcribed faithfully, flag for correction. **H5** Imam is uncertain in the source itself — "from Ali B. al-Husayn (al-Sajjad), **or** from Abu Ja'far (al-Baqir)" (rendered with the "or" preserved). **Abu Hamza al-Thumali** (H1, H5) spelling normalised from hubeali's "al-Sumali" to the Arabic الثُّمَالِيّ. The English re-translation also gets the same scholar review. (Provenance: ship behind the `ai_translation_flag` pattern.)
- **Progress:** **Part 1 Ch 1–5 = 40 narrations done** (Ch 1–2 originally `cdd399b`; Ch 3 + first lead pass `5b82a09`; then the closest-to-Imam re-lead of all 25 + 1-1-5 non-Imam + footnote + scholar conventions; **Ch 4 "Merits of the Scholar over the Worshipper" = 10 narrations (`basair-1-4-1`…`1-4-10`), pages 28–30**; **Ch 5 "The people are categorised upon three…" = 5 narrations (`basair-1-5-1`…`1-5-5`), pages 31–32 — FIRST team-sourced chapter** (drafted by the user on claude.ai Sonnet via the translator kit, gate-corrected + ingested here). Part 1 has 24 chapters; the book has ~10 parts (~1,800 narrations). **Next: Ch 6** (PDF p.33+) — **only on the user's say-so**. The local backup `concept-ui/public/basair_complete.BACKUP-20260630.json` and `kisamotif.jpg` were intentionally left untracked (not committed).
- **Ch 5 gate corrections (what the Sonnet draft got wrong — pattern to watch when ingesting team output):** (1) **Arabic verb misread:** the matn verb is **يَغْدُو/يَغْدُونَ** ("people fall into / come forth as three kinds"), which Sonnet misread/guessed as يُعَدُّون ("are counted"); it correctly *flagged the uncertainty in NOTES* (the kit's "flag, don't silently fix" worked) but its suggested fix was wrong — verified against page image. (2) **Chain name:** `Abd al-Rahman` (عَبْدِ الرَّحْمَنِ) misread as "Abi al-Rahman" in 1-5-3. (3) **House spellings normalised on ingest:** `al-` lowercased, Jameel, al-Abbas B. Aamir, Sayf B. Ameyra, Abd al-Hameed, al-Washsha. **Lead rule, Imam IDs, tokens, structure were all correct in the draft** — the gate pass is mainly Arabic-reading + spelling normalisation.

- **Distributed translation (team scale-up, set 2026-06-30):** to beat the owner's usage limits, teammates translate chapters on their own **claude.ai Pro/Team** workspaces and paste results back. Three artefacts created: (1) **`docs/basair-translator-kit.md`** — self-contained house-style instructions + fielded output format; goes into a shared claude.ai **Project**'s custom instructions. (2) **`docs/basair-translator-quickstart.md`** — non-technical owner-setup + translator-loop guide. (3) **`~/Documents/Basair-Chapters/`** — one **image-only** PDF per chapter (183 files, 10 parts; corrupted Arabic text layer rasterised away so claude.ai reads clean page images), plus `ASSIGNMENT-INDEX.csv` (part/chapter/pages/file) and `_regenerate_chapter_pdfs.py` (the splitter; auto-detects chapters via the intact **English** text layer — TOC pages 2–18 skipped, body starts "PART ONE" p.19, chapter numbers reset per part). Hand out from **Part 1 Ch 5** onward.
- **Ingesting team translations (procedure):** translators emit blocks with fields **CHAIN / BODY / ARABIC / NOTES** per hadith under a `PART n — CHAPTER n / TITLE:` header. Map to schema (same as `build_ch4.py` template): `manual_chain` = `thaqalaynSanad` = CHAIN; `manual_body` = BODY; `englishText` = `"{n}. " + BODY`; `arabicText` = ARABIC; `thaqalaynMatn` = **derive** = the quoted saying from BODY (after the blank line), or for a *mithl* hadith a short note like "(The same report as Hadith X.)"; `chapterInCategoryId` = chapter no.; `id` = `basair-1-{ch}-{n}`; `chapter` = `"Chapter {ch}: {Title Case}"`; constants (`bookId`, `book`, `volume`="Part 1", `category`, `author`, `translator`) copied from existing records; set `nonImamReport: true` if NOTES say so. **Always re-validate against the house style before appending** (lead rule, tokens, `\n\n` matn break, `B.` not Bin) — the team output is a draft, not authoritative; the owner is the gate. Append to `concept-ui/public/basair_complete.json`, verify JSON + counts, commit to `main`. **Clarifier policy (decided 2026-06-30): KEEP Imam `(clarifier)` annotations throughout the whole book** for consistency now that many hands are involved (the earlier "retire after Ch5" idea is shelved; the owner may bulk-omit later if desired). Distribution model: **each translator uses their OWN private claude.ai project** (not a shared one — avoids cross-visibility); dedup is enforced by the owner's **`ASSIGNMENT-TRACKER.csv`** (in `~/Documents/Basair-Chapters/`), not by Claude.
