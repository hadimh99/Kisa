...
> **▶ RESUME HERE (next action):** Part 1 **Ch 1 (5) + Ch 2 (16) = 21 narrations are committed** (`cdd399b`). **Ch 3 (4 narrations: "Recognition of the Scholar…", PDF pp.26–27) is now also done and verified** (appended to `basair_complete.json`, renders correctly at the deep link below; honorific tokens + matn line-break confirmed). **Ch 3 is NOT yet committed** — the user runs `git add`/commit/push themselves. **Next is Ch 4** (PDF p.27+) — **wait for the user's explicit go-ahead**. To continue: re-read this subsection (the conventions + pipeline below), then render PDF pages with PyMuPDF (`venv/bin/python`, `fitz.Matrix(4.5,4.5)` zoomed band crops read cleanly), read the Arabic/English visually, translate, and append using the same `manual_chain`/`manual_body` format. Verify in the app (dev server, `/hadith/basair-al-darajat/vol-1/part-1/<slug>`; slug = `slugifyHadithParam(chapter)` from `HadithLibrary.jsx`).
...
- **House style for honorifics (translation conventions):**
  ...
  - **Narrator-led attribution format (set 2026-06-30):** the displayed body must **lead with the most-recent narrator** (al-Ṣaffār's direct source — the first link in the isnad), rendered **`B.` not `Bin`**, then flow to the speaker. Connectors: Imam→Prophet = *"…narrates that Abu Ja'far (al-Baqir) {{AS}}, narrates that Rasulullah {{SAW}} said:"*; Imam via his father (an Imam) = *"…narrates that Abu Abdullah (al-Sadiq) {{AS}}, from his father, Abu Ja'far (al-Baqir) {{AS}}, who said: Rasulullah {{SAW}} said:"* (**name the father Imam**, don't leave "his father" vague); Imam speaking directly = *"…is reported to have said:"*; **Imam relaying another Imam** (e.g. al-Sadiq via Amir al-Mu'minin) = slim it to *"…from Ameer al-Mu'mineen (Imam Ali) {{AS}}, who said:"* — **do not stack two "narrates that"** (the only kept double is Imam→Prophet, e.g. H8). **Lead-narrator precedence (revised 2026-06-30):** if a **famous narrator** appears **anywhere in the chain**, the body **leads with that name** — it takes precedence over the actual direct hearer AND over al-Ṣaffār's first link. The famous list (extend on request): **Abu Basir, Jabir (al-Juʿfī), Muhammad B. Muslim, Ibn Abi Umayr** (note "Abu Basir" spelling). If **multiple** famous names appear, lead with the one **closest to the Imam** / the actual hearer (e.g. H13 has both Ibn Abi Umayr and Abu Basir → lead with Abu Basir). If **none** are present, fall back: first-person hearing ("I heard X" / "X narrated to me") → lead with the direct hearer; otherwise → al-Ṣaffār's first link. *(Ibn Abi Umayr — Muhammad b. Abi Umayr — is a top-tier* thiqa *of the* Aṣḥāb al-Ijmāʿ; *he sits mid-chain in these early al-Bāqir/al-Ṣādiq isnads since he lived in the 7th–9th Imams' era, yet still leads the body per this rule. [wikishia](https://en.wikishia.net/view/Muhammad_b._Abi_Umayr))* The connectors above still apply once the lead name is chosen. The **full isnad** still goes in `manual_chain` (the collapsible "View Chain of Narrators"); `B.` is used there too. **Implementation:** authored via explicit **`manual_chain` + `manual_body`** fields on each record (the reader's `hadith.manual_body || parsedBody` path bypasses `splitText`), so display is exact rather than regex-parsed. `englishText` keeps a `"N. " + body` copy for the hadith-number badge + search.
