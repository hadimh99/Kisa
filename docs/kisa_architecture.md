...
- **Progress:** **Part 1 Ch 1–4 = 35 narrations done** (Ch 1–2 originally `cdd399b`; Ch 3 + first lead pass `5b82a09`; then the closest-to-Imam re-lead of all 25 + 1-1-5 non-Imam + footnote + scholar conventions; **Ch 4 "Merits of the Scholar over the Worshipper" = 10 narrations (`basair-1-4-1`…`1-4-10`), pages 28–30, added this session** — chapter slug `chapter-4-merits-of-the-scholar-over-the-worshipper`). Part 1 has 24 chapters; the book has ~10 parts (~1,800 narrations). **Next: Ch 5** (PDF p.31+) — **only on the user's say-so**. The local backup `concept-ui/public/basair_complete.BACKUP-20260630.json` and `kisamotif.jpg` were intentionally left untracked (not committed).
