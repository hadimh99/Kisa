    ...
    return (
        <div className="flex flex-col gap-6">
            <div className="flex justify-between items-end border-b border-zinc-800 pb-4">
                ...
                <div className="flex bg-[#121212] p-1 rounded-xl border border-zinc-800 shadow-xl overflow-hidden">
                    <button
                        onClick={() => { setActiveTab('queue'); setExpandedRowId(null); setStatus({ type: '', message: '' }); }}
                        className={`px-6 py-2.5 text-xs font-bold tracking-widest uppercase transition-all whitespace-nowrap rounded-lg ${activeTab === 'queue' ? 'bg-zinc-800/80 text-[#c6a87c] shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}
                    >
                        Editing Queue
                    </button>
                    <button
                        onClick={() => { setActiveTab('activity'); setExpandedRowId(null); setStatus({ type: '', message: '' }); }}
                        className={`px-6 py-2.5 text-xs font-bold tracking-widest uppercase transition-all whitespace-nowrap rounded-lg ${activeTab === 'activity' ? 'bg-zinc-800/80 text-[#c6a87c] shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}
                    >
                        Activity & Trash
                    </button>
                </div>
            ...
            {!metricsLoading && metrics.total > 0 && (
                <div className="bg-[#121212] border border-zinc-800 rounded-2xl shadow-xl p-6 lg:p-8 shrink-0">
                    <div className="flex flex-col lg:flex-row justify-between items-center gap-10">
                        <div className="flex-1 w-full">
                            <div className="flex justify-between items-end mb-3">
                                <div>
                                    <div className="text-3xl font-black text-white">{((metrics.completed / metrics.total) * 100).toFixed(1)}% <span className="text-zinc-500 font-light text-xl">Comp</span></div>
                                    <div className="text-xs font-mono text-[#c6a87c] tracking-widest uppercase mt-2">
                                        {metrics.completed.toLocaleString()} / {metrics.total.toLocaleString()} Narrations
                                    </div>
                                </div>
                                <div className="text-right flex flex-col items-end justify-end">
                                    <div className="text-sm text-zinc-300 font-bold tracking-wide">
                                        {metrics.etaDays === Infinity ? 'ETA: Need more data' : `Est. Completion: ${metrics.etaDays.toLocaleString()} Days`}
                                    </div>
                                    <div className="text-[10px] text-zinc-500 uppercase tracking-widest mt-1 bg-zinc-900 px-2 py-1 rounded inline-block w-max">
                                        Velocity: {metrics.velocity} Edits/Day
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="w-full lg:w-64 h-20 flex items-end justify-between gap-1.5 pt-4">
                            {metrics.sparkline.map((count, i) => {
                                ...
                                return (
                                    <div key={i} className="flex-1 flex flex-col justify-end items-center group relative h-full">
                                        ...
                                        ></div>
                                        <div className="absolute -top-10 bg-[#1c1c1e] shadow-xl border border-zinc-700 text-white text-[10px] font-mono px-3 py-1.5 rounded opacity-0 group-hover:opacity-100 pointer-events-none z-10 transition-opacity whitespace-nowrap">
                                            {count} edits
                                        </div>
                                    </div>
                                )
                        ...
                        {isMatrixOpen && (
                            ...
                            >
                                <div className="space-y-6 pt-6">
                                    <div className="text-xs font-bold uppercase tracking-widest text-[#c6a87c] mb-4 flex items-center justify-between">
                                        Active Volumes
                                        <span className="text-[10px] bg-zinc-900 px-2 py-1 rounded text-zinc-500">Auto-Archives at 100%</span>
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                                        {Object.entries(metrics.matrix).map(([bookName, volumes]) => {
                                            return Object.entries(volumes).map(([volName, stats]) => {
                                                ...
                                                return (
                                                    <div key={`active-${bookName}-${volName}`} className="bg-black/30 border border-zinc-800/80 hover:border-zinc-700 transition-colors rounded-xl p-5 shadow-sm">
                                                        <div className="flex justify-between items-center mb-3">
                                                            <div className="text-sm font-semibold text-zinc-300 truncate pr-2" title={bookName}>{bookName}</div>
                                                            <div className="text-[10px] font-mono text-[#c6a87c] bg-[#c6a87c]/10 border border-[#c6a87c]/20 px-2 py-1 rounded whitespace-nowrap">Vol {volName}</div>
                                                        </div>
                                                        <div className="flex justify-between items-end mb-2">
                                                            <span className="text-xs font-bold text-white">{pct.toFixed(0)}%</span>
                                                            <span className="text-[10px] text-zinc-500 font-mono tracking-widest">{stats.completed} / {stats.total}</span>
                                                        </div>
                                    ...
                                    <details className="mt-10 group border border-zinc-800/60 rounded-xl overflow-hidden bg-black/40 transition-all">
                                        <summary className="cursor-pointer p-4 text-xs font-bold uppercase tracking-widest text-zinc-400 hover:text-white transition-colors flex items-center justify-between list-none">
                                            <div className="flex items-center gap-3">
                                                <Save className="w-4 h-4 text-green-500/70 group-open:text-green-500" />
                                                Completed Archives Vault
                                            </div>
                                            <div className="text-[10px] bg-green-900/20 border border-green-900/40 text-green-400 px-2 py-1 rounded">Locked</div>
                                        </summary>
                                        <div className="p-5 border-t border-zinc-800/50 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3 bg-[#121212]">
                                            {Object.entries(metrics.matrix).map(([bookName, volumes]) => {
                                                return Object.entries(volumes).map(([volName, stats]) => {
                                                    const pct = (stats.completed / stats.total) * 100;
                                                    if (pct < 100) return null;
                                                    return (
                                                        <div key={`archived-${bookName}-${volName}`} className="border border-green-900/40 bg-green-900/10 rounded-lg p-3 flex justify-between items-center transition-opacity hover:opacity-80">
                                                            <div className="truncate text-[11px] text-zinc-400 font-semibold pr-2" title={`${bookName} Volume ${volName}`}>{bookName} v{volName}</div>
                                                            <span className="text-[9px] font-bold text-green-400 bg-green-900/40 px-2 py-0.5 rounded uppercase tracking-wider">Mastered</span>
                                                        </div>
                                                    );
                                                });
                                            })}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-2">
                <div className="flex gap-2">
                    <button onClick={() => setReviewQueueMode(false)} className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all ${!reviewQueueMode ? 'bg-[#c6a87c] text-black shadow-lg shadow-[#c6a87c]/20' : 'bg-[#121212] border border-zinc-800 text-zinc-500 hover:text-white hover:border-zinc-700'}`}>Standard Queue</button>
                    <button onClick={() => setReviewQueueMode(true)} className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all flex items-center gap-2 ${reviewQueueMode ? 'bg-amber-500 text-black shadow-xl shadow-amber-500/20' : 'bg-[#121212] border border-zinc-800 text-amber-500/50 hover:text-amber-500 hover:border-amber-900/50'}`}>Review Queue <AlertCircle className="w-3.5 h-3.5" /></button>
                </div>
                ...
                {reviewQueueMode && (
                    <select
                        ...
                        className="bg-[#1c1c1e] text-amber-500 text-xs font-bold uppercase tracking-widest border border-amber-900/50 rounded-lg px-4 py-2 outline-none shadow-inner cursor-pointer"
                    >
            ...
            {!reviewQueueMode && (
                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3 bg-[#121212] p-5 border border-zinc-800 rounded-2xl shadow-xl mt-1">
                    <select
                        value={bookFilter}
            ...
            {status.message && (
                <div className={`p-4 rounded-xl flex items-center text-sm font-medium shadow-md ${status.type === 'error' ? 'bg-red-900/20 text-red-400 border border-red-900/50' :
            ...
            {(() => {
                const renderEditorPanel = (item) => (
                    <div
                        className="col-span-12 bg-[#1c1c1e] border-y border-[#c6a87c]/30 shadow-inner p-6 sm:p-10 cursor-default focus:outline-none"
                    ...
                    >
                        ...
                        <div className="flex flex-col gap-4 bg-amber-900/10 border border-amber-500/30 p-5 rounded-xl mb-8 shadow-inner">
                            <div className="flex items-center justify-between">
                                <label className="text-[11px] font-bold uppercase tracking-[0.2em] text-amber-500 flex items-center gap-2 cursor-pointer">
                                    <input
                                        type="checkbox"
                                    ...
                                    />
                                    Flag for Review / Collaboration
                                </label>
                                {formData.is_flagged && (
                                    <select
                                        value={formData.assigned_to || 'UNASSIGNED'}
                                        onChange={(e) => handleFormChange('assigned_to', e.target.value)}
                                        className="bg-black/50 border border-amber-500/30 text-amber-500 text-xs font-bold uppercase tracking-widest rounded px-3 py-1.5 outline-none cursor-pointer hover:border-amber-500/60 transition-colors"
                                    >
                                        <option value="UNASSIGNED">Unassigned</option>
                                        <option value="MASTER ADMIN">Master Admin</option>
                                        <option value="CO-ADMIN (PENDING)">Co-Admin (Pending)</option>
                            ...
                            <textarea
                                ...
                                className="w-full bg-black/40 border border-amber-500/20 hover:border-amber-500/50 focus:border-amber-500/80 rounded-lg p-3 text-zinc-300 font-sans text-sm resize-y outline-none transition-all"
                                rows={2}
                            />
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                            <div className="flex flex-col gap-8">
                                ...
                                {/* Safely Isolated Auto-Formatter Tool */}
                                <div className="bg-blue-900/10 border border-blue-900/30 rounded-xl p-5 mb-2">
                                    <div className="flex items-start gap-4">
                                        <div className="bg-blue-900/20 p-2.5 rounded-lg shrink-0 text-blue-400">
                                            ✨
                                        </div>
                                        <div className="flex-1">
                                            <h4 className="text-xs font-bold uppercase tracking-widest text-blue-400 mb-1">Magic Wand Formatting Tool</h4>
                                            <p className="text-xs text-zinc-400 mb-4 leading-relaxed">
                                                ...
                                                <br /><span className="text-amber-500/80 mt-1 block">Note: This does not save the text. You can review the changes in the box below before committing.</span>
                                            </p>
                                            <button
                                                onClick={handleMagicWand}
                                                className="px-5 py-2.5 bg-blue-900/20 hover:bg-blue-900/40 text-blue-400 border border-blue-900/50 rounded-lg text-xs font-bold tracking-wider transition-colors shadow-sm"
                                            >
                                                Run Auto-Format Now
                                            </button>
                                        </div>
                        ...
                        <div className="mt-10 pt-6 border-t border-[#c6a87c]/20 flex justify-between items-center flex-wrap gap-4">
                            <div className="flex items-center gap-3">
                                <button
                                    onClick={() => handleSoftDelete(item.id)}
                                    disabled={status.type === 'loading'}
                                    className="px-6 py-3 font-bold uppercase tracking-widest text-xs text-red-500 hover:text-red-400 hover:bg-red-900/20 rounded transition-colors"
                                >
                                    Trash Hadith
                                </button>
                            </div>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <button
                                    ...
                                    disabled={status.type === 'loading'}
                                    className="bg-zinc-800 hover:bg-zinc-700 text-white font-bold uppercase tracking-widest text-xs px-6 py-3 rounded-md shadow-lg transition-all flex items-center gap-2 border border-zinc-600"
                                >
                                    Save as Draft
                                </button>
                                ...
                                <button
                                    ...
                                    className="bg-zinc-800 hover:bg-zinc-700 text-white font-bold uppercase tracking-widest text-xs px-6 py-3 rounded-md shadow-lg transition-all flex items-center gap-2 disabled:opacity-50 border border-transparent hover:border-zinc-500"
                                >
                                    Save & Next
                                </button>
                                ...
                                <button
                                    ...
                                    disabled={status.type === 'loading'}
                                    className={`${formData.is_flagged ? 'bg-amber-500 hover:bg-amber-400 text-amber-950 shadow-amber-500/20' : 'bg-[#c6a87c] hover:bg-[#b0956b] text-black'} font-bold uppercase tracking-widest text-xs px-8 py-3 rounded-md shadow-lg transition-all flex items-center gap-2 disabled:opacity-50`}
                                >
                                    {formData.is_flagged ? <Send className="w-4 h-4" /> : <Save className="w-4 h-4" />}
                                    {status.type === 'loading' ? 'Syncing...' : formData.is_flagged ? 'Send to Review' : 'Save Hadith'}
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                return (
                    activeTab === 'queue' ? (
                                ...
                                {loading && hadiths.length === 0 ? (
                                    ...
                                    hadiths.map((item) => (
                                        <React.Fragment key={item.id}>
                                            <div
                                                className={`grid grid-cols-12 gap-4 p-4 border-b border-zinc-800/50 items-center transition-colors cursor-pointer ${expandedRowId === item.id ? 'bg-[#1a1a1c] border-l-[3px] border-l-[#c6a87c]' :
                                            ...
                                            >
                                                ...
                                                <div className="col-span-4 sm:col-span-6 flex flex-col justify-center">
                                                    <div className="text-sm text-zinc-400 truncate pr-4">{item.englishText || 'No translation provided.'}</div>
                                                    {item.internal_notes && (
                                                        <div className="mt-2 bg-amber-900/20 border-l-[3px] border-amber-500 p-2 rounded-r-lg max-w-[80%]">
                                                            <div className="flex items-center justify-between mb-1">
                                                                <span className="text-[10px] font-bold text-amber-500 uppercase tracking-widest">Internal Note</span>
                                                                <span className="text-[10px] text-zinc-500 font-mono">By {item.last_edited_by?.split('@')[0] || 'Unknown'} | {item.assigned_to || 'Unassigned'}</span>
                                                            </div>
                                                            <p className="text-xs text-amber-100/80 italic line-clamp-2">{item.internal_notes}</p>
                                                        </div>
                                                    )}
                                                </div>
                        ...
                        <div className="bg-[#121212] border border-zinc-800 rounded-2xl overflow-hidden shadow-2xl flex flex-col h-[70vh]">
                            <div className="flex-1 overflow-y-auto p-4" style={{ scrollbarWidth: 'thin', scrollbarColor: '#3f3f46 transparent' }}>
                                        ...
                                        {activityLog.map(item => (
                                            <React.Fragment key={item.id}>
                                                <div
                                                    ...
                                                    className={`p-4 border rounded-xl flex justify-between items-center transition-all ${item.is_trashed ? 'bg-red-900/5 border-red-900/30' :
                                                ...
                                                >
                                                    <div>
                                                        <div className="flex items-center gap-3 mb-1">
                                                            <span className="text-zinc-300 font-mono text-sm">{item.book} • Vol {item.volume} • Num {item.hadith_number}</span>
                                                            {item.is_trashed ? (
                                                                <span className="bg-red-900/30 text-red-500 text-[10px] font-bold px-2 py-0.5 rounded shadow border border-red-900/50">TRASHED</span>
                                                            ) : (
                                                                <span className="bg-[#c6a87c]/10 text-[#c6a87c] text-[10px] font-bold px-2 py-0.5 rounded shadow border border-[#c6a87c]/30">EDITED</span>
                                                            )}
                                                        </div>
                                                        <div className={`text-sm truncate max-w-2xl ${item.is_trashed ? 'text-zinc-600 line-through' : 'text-zinc-400'}`}>
                                                    ...
                                                    <div className="flex items-center gap-4 ml-4">
                                                        ...
                                                        {item.is_trashed ? (
                                                            <button
                                                                ...
                                                                className="bg-zinc-800 hover:bg-zinc-700 text-white text-xs font-bold px-4 py-2 rounded-lg transition-colors border border-zinc-600 shadow"
                                                            >
                                                                Restore
                                                            </button>
                                                        ) : (
