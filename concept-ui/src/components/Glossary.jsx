                                                    ...
                                                    return (
                                                        <div key={uniqueKey} className={`border p-5 sm:p-6 rounded-2xl transition-colors group relative shadow-sm hover:border-[#c6a87c]/50 ${colors.cardBg} ${colors.border}`}>
                                                            <div className="absolute top-4 right-4 flex items-center gap-1 sm:gap-2">
                                                                <button aria-label="Share term" onClick={() => handleShare(term)} className={`p-2 transition-colors rounded-full ${colors.textSecondary} hover:bg-[#c6a87c]/10 hover:text-[#c6a87c]`}><Share2 className="w-4 h-4" /></button>
                                                                <button aria-label="Copy term" onClick={() => handleCopy(term, uniqueKey)} className={`p-2 transition-colors rounded-full ${colors.textSecondary} hover:bg-[#c6a87c]/10 hover:text-[#c6a87c]`}>{copiedId === uniqueKey ? <Check className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4" />}</button>
                                                                <button aria-label="Save term to Study Vault" onClick={() => handleSaveToVault(term, uniqueKey)} disabled={savingId === uniqueKey} className={`p-2 transition-colors rounded-full ${savingId === uniqueKey ? 'text-[#c6a87c]' : colors.textSecondary} hover:bg-[#c6a87c]/10 hover:text-[#c6a87c]`}><Bookmark className={`w-4 h-4 ${savingId === uniqueKey ? 'fill-[#c6a87c]' : ''}`} /></button>
                                                            </div>
